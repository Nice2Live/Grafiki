...
```
## Обработка файла txt (файл должен содержать структуру словаря(dict) "{key: json}")
```python
...
#суммируем оценки указанных предметов в один конкретный предмет
...
```
## Создаём круговую диограмму с общим количеством оценок
```python
# --- Круговая диаграмма ---
total_counts = {
    "5": int(df["5"].sum()),
...
plt.show()
```
## Создаём столбчатый график с оценками по предметам
```python
subjects = df["subject"]
...
plt.show()
```
## Классификация предметов на категории: Физико-Математические и Гуманитарные
```python 
...
if not category_df.empty:
    ...
    plt.show()
```
## Вычисление корреляции между предметами и Топ 5 лучших, Топ 5 худших корреляций
```python
